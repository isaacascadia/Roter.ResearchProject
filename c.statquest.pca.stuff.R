...
# expects samples to be rows, and genes to be columns (transposition)
pca <- prcomp(t(data.matrix), scale=TRUE)  # returns x (principal components)
...
## make a scree plot
pca.var <- pca$sdev^2                              # calculate variation
pca.var.per <- round(pca.var/sum(pca.var)*100, 1)  # calculate percentage of var
